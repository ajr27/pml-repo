...
Finally, I'll predict the outcomes of the cases in the "pml-test" data using the random forest model developped above and save them for submission.
```{r predict, echo = TRUE}
final.pred <- predict(fitRF, validation, type = "class")

dir.create("./predictions/", showWarnings = FALSE, recursive = TRUE)
setwd("./predictions/")

# Create prediction files
invisible(sapply(seq_along(final.pred), function(x) {
    write.table(final.pred[x], file = paste0("problem_id_", x, ".txt"),
